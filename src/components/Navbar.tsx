      ...
      >
        <div className="max-w-[1200px] mx-auto px-4 md:px-8 h-[68px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              <div className="flex items-center gap-2">
                <span className="hidden sm:block font-body text-xs text-pg-gray-500 truncate max-w-[120px]">{user.email}</span>
                <button onClick={signOut}
                  className="px-4 py-2 rounded-xl font-body text-sm font-medium text-pg-gray-700 hover:text-destructive transition-colors"
              ...
              <button onClick={() => setAuthOpen(true)}
                className="px-5 py-2.5 rounded-xl bg-primary text-primary-foreground font-body text-sm font-semibold shadow-pg-btn hover:shadow-pg-btn-hover hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98] transition-all">
                {t.nav.startScanning}
              </button>
            )}
          </div>
